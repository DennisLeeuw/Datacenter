\documentclass[a4paper,12pt,twoside,openright,titlepage]{book}

%Additional packages
\usepackage[ascii]{inputenc}
\usepackage[T1]{fontenc}
% textcomp needed for degree sign
\usepackage{textcomp}
\usepackage[dutch,english]{babel}
\usepackage{syntonly}
\usepackage[official]{eurosym}
%\usepackage[graphicx]
\usepackage{graphicx}
\graphicspath{ {./images/} }
\usepackage{float}
\usepackage{xurl}
\usepackage{hyperref}
\hypersetup{colorlinks=true, linkcolor=blue, citecolor=blue, filecolor=blue, urlcolor=blue, pdftitle=, pdfauthor=, pdfsubject=, pdfkeywords=}
\usepackage{tabularx}
\usepackage{scrextend}
\addtokomafont{labelinglabel}{\sffamily}
\usepackage{listings}
\usepackage{adjustbox}
\usepackage{epigraph}

% Turn on indexing
\usepackage{imakeidx}
\makeindex[intoc]

% Define colors
\usepackage{color}
\definecolor{ashgrey}{rgb}{0.7, 0.75, 0.71}

% Listing style
\lstset{
  backgroundcolor=\color{ashgrey},   % choose the background color; you must add \usepackage{color} or \usepackage{xcolor}; should come as last argument
  basicstyle=\footnotesize,        % the size of the fonts that are used for the code
  breakatwhitespace=false,         % sets if automatic breaks should only happen at whitespace
  breaklines=true,                 % sets automatic line breaking
  extendedchars=true,              % lets you use non-ASCII characters; for 8-bits encodings only, does not work with UTF-8
  frame=single,	                   % adds a frame around the code
  keepspaces=true,                 % keeps spaces in text, useful for keeping indentation of code (possibly needs columns=flexible)
  rulecolor=\color{black},         % if not set, the frame-color may be changed on line-breaks within not-black text (e.g. comments (green here))
  showspaces=false,                % show spaces everywhere adding particular underscores; it overrides 'showstringspaces'
}

% Commands
%\newcommand*\chem[1]{\ensuremath{\mathrm{#1}}}

% Uncomment for production
% \syntaxonly

% Style
\pagestyle{headings}

%%%%%%%%%%%%%%%%%%
% Begin document %
%%%%%%%%%%%%%%%%%%

% Define document
\author{D. Leeuw}
\title{Datacenter inrichting}
\date{\today\\Versie 1.0.0}

\begin{document}
\selectlanguage{dutch}

\maketitle

\epigraph{quis custodiet ipsos custodes (Wie bewaakt de bewakers)?}{\textit{Juvenal}}

\newpage

\copyright\ 2021 Dennis Leeuw\\

\begin{figure}[H]
\includegraphics[width=0.3\textwidth]{CC-BY-SA-NC.png}
\end{figure}

\bigskip

\input{src/CC-licentie}

%%%%%%%%%%%%%%%%%%%
%%% Introductie %%%
%%%%%%%%%%%%%%%%%%%

\frontmatter
\chapter{Over dit Document}
\input{src/DocOver}
\input{src/DocChanges}

%%%%%%%%%%%%%%%%%
%%% De inhoud %%%
%%%%%%%%%%%%%%%%%
\tableofcontents

\mainmatter

\chapter{Het datacenter en de serverruimte}\index{Datacenter}\index{Serverruimte}
\input{src/dc_intro}
\section{De inrichting van een datacenter}
\input{src/dc_inrichting}
\section{DER, SER en MER}
\input{src/dc_types}
\section{De vloer}
\input{src/dc_vloer}

\chapter{Een datacenter cabinet}
\input{src/dc_cabinet}
\section{De kast}
\input{src/dc_kasten}
\section{De servers}
\input{src/server_hardware}
\section{Stroomvoorziening}
\input{src/cabinet_PDU}
\section{Netwerkbekabeling}
\input{src/cabinet_network}

\chapter{Netwerk voorzieningen}
\section{Hi\"erarchisch Netwerk Ontwerp}
\input{src/HND.tex}
\section{Patch-panelen}
\input{src/patchpaneel}
\section{Top of Rack Switches}
\input{src/top_of_rack_switches}
\section{Netwerk bekabeling}
\input{src/dc_bekabeling}

\chapter{Voeding}
\input{src/dc_power}
\section{Electriciteitsnet}
\input{src/dc_power_elecnet}
\section{UPS}
\input{src/dc_power_ups}
\subsection{Batterijen}
\input{src/dc_power_ups_batt}
\subsection{Vliegwiel}
\input{src/dc_power_ups_flywheel}
\section{Generator}
\input{src/dc_generator}
\section{Distributie paneel}
\input{src/dc_mdb}
\section{PUE}
\input{src/dc_power_pue}

\chapter{Koeling}
\input{src/dc_koeling}
\section{Lucht circulatie}
\input{src/dc_koeling_stroming}
\section{Temperatuur}
\input{src/dc_koeling_temperatuur}
\section{Luchtvochtigheid}
\input{src/dc_koeling_luchtvochtigheid}
\section{Koeltechnieken}
\subsection{Compressorkoeling}
\input{src/dc_koeling_compressor}
\subsection{Adiabatische koeling}
\input{src/dc_koeling_adiabatisch}
\subsection{PCM koeling}
\input{src/dc_koeling_pcm}
\subsection{Vloeistofkoeling}
\input{src/dc_koeling_vloeistof}

\chapter{Beveiliging}
\section{Redundantie}
\input{src/dc_redundancy}
\subsection{N+1}
\input{src/dc_redundancy_N1}
\subsection{2N}
\input{src/dc_redundancy_2N}
\subsection{2(N+1)}
\input{src/dc_redundancy_2N1}
\subsection{xN/y}
\input{src/dc_redundancy_xNy}
\section{TIA-942 Datacenter Classificatie}
\input{src/dc_TIA942_Tier}
\subsection{Tier-1}
\input{src/dc_TIA942_Tier1}
\subsection{Tier-2}
\input{src/dc_TIA942_Tier2}
\subsection{Tier-3}
\input{src/dc_TIA942_Tier3}
\subsection{Tier-4}
\input{src/dc_TIA942_Tier4}
\section{Toegangsbeveiliging}
\input{src/dc_security_physical}
\section{Calamiteiten bewaking en beveiliging}
\input{src/dc_security_sensors}
\input{src/dc_security_fire}

\chapter{Onderhoud}
\input{src/dc_onderhoud_intro}
\section{Soorten onderhoud}
\input{src/dc_onderhoud_soorten}
\section{Stof}
\input{src/dc_onderhoud_stof}
\section{Blusmiddelen}
\input{src/dc_onderhoud_blusmiddelen}
\section{Koeling}
\input{src/dc_onderhoud_koeling}
\section{Voeding}
\input{src/dc_onderhoud_voeding}
\section{Veiligheid}
\input{src/dc_onderhoud_veiligheid}
\section{Component controle}
\input{src/dc_onderhoud_componenten}

%\chapter{TCO}\index{TCO}\index{Total Cost of Ownership}
% Boekjes virt TCO
% Boek Security in systemen en netwerken MTBF
% highavailability en scalability???
% Voordelen outsourcen
% Nadelen outsourcen (opdracht?)

%%%%%%%%%%%%%%%%%%%%%
%%% Index and End %%%
%%%%%%%%%%%%%%%%%%%%%
%\backmatter
\printindex
\end{document}

%%% Last line %%%
